...
class PluginController extends RenoController
{
    ...
    public function index($project)
    {
        try {
            ...
            return $this->render('plugin_index.html.twig', [
                    'project' => $project_obj,
    ...
    {
        $plugin_entity = $this->ds->queryOne('\\App\\Entity\\Plugin', [
            'project' => $project,
            'name' => $plugin,
        ]);
        $pluginCore = null;
            ...
            case $pluginAction::HANDLEBY_RENDER:
                ...
                if ($pluginAction->getAction() == 'configure') {
                    $this->addCrumb($pluginCore->getName(), $this->nav->path("app_plugin_configure", $this->nav->entityParams($project_obj)
                            + ['plugin' => $pluginCore->getName()]), $pluginCore->getIcon());
                } else {
                    $this->addCrumb($pluginCore->getName(), $this->nav->path("app_plugin_action", $this->nav->entityParams($project_obj)
                            + ['plugin' => $pluginCore->getName(), 'action' => $pluginAction->getAction()]), $pluginCore->getIcon());
                }
                return $this->render("@plugin/".$pluginCore->getName()."/".$pluginAction->getRenderView(), array_merge_recursive([
                        'project' => $project_obj,
                        'core' => $pluginCore,
                        'plugin' => [
